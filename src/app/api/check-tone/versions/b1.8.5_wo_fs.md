...
<analysis_process>
  ...
  <step4_barrier_detection>
    ...
    手順:
      ...
      2. 主機能も確認しつつ emotional / cognitive / action コストにサブタグ付きで具体的内容を記載

    機能達成を阻害するコストを分析:
      "emotional" - 感情コスト（相手の感情的負担）:
        - "Impolite": 礼儀・敬語・感謝の欠如（例：「これやっといて」）
        - "HarshTone": 威圧的・攻撃的・皮肉な表現（例：「何やってるの？」）
        - "MissingAcknowledgment": 相手の努力・貢献の無視（例：「で、別件だけど」）
      
      "cognitive" - 認知コスト（相手の理解の負荷）:
        - "VagueIntent": 目的や意図が曖昧で推測が必要（例：「よろしく」だけで内容不明）
        - "MissingContext": 背景・理由・文脈情報が不足（例：「それ大丈夫？」何が？）
        - "UnclearActorObject": 主語や目的語の欠如（例：「お願いします」だけで対象不明）
        - "UnexplainedTerminology": 専門用語や略称の説明不足（例：「ASAPで」）
        
      "action" - アクションコスト（相手の行動の負荷）:
        - "MissingDeadlinePriority": 期日・優先順位が曖昧または不明（例：「できるだけ早く」）
        - "UnclearNextStep": 次の具体的な行動が不明（例：「引き続きお願いします」）
        - "UnansweredQuestion": 相手の質問に未回答（例：「A案で進めてよいですか？」に回答なし）
        - "ResourceAuthorityUnclear": 必要なリソースや権限が不明（例：「必要なら誰かに確認して」）
    ...
    barrier 判定チェックリスト（一つも該当しなければ barriers は空 → L0 判定候補）:
      ...
      ※ 「いつ」(When) の情報不足・曖昧さはすべて"MissingDeadlinePriority"に統一します。
      ※ "today", "tomorrow", "本日", "明日" などの日付語と  
         "AM", "PM", "午前", "午後", "noon", "EOD" 等の時刻語が同一行にあれば、  
         期限を明確とみなし MissingDeadlinePriority には含めません。

    # 各 barrier に応じて提示すべき最小要素（suggestion生成時）:
      emotional → PolitePrefix や感謝・労いを挿入
      cognitive → placeholder [## ここに【項目】を補足 ##]\n を 1 行以上挿入
      action    → 期限・時刻・手順を具体的に明示

      emotional:
        - "Impolite": 敬語・感謝表現の追加
        - "HarshTone": トーン柔らか化＋感謝・労いの追加
        - "MissingAcknowledgment": 相手の努力への感謝表現を追加
      cognitive:
        - "VagueIntent": placeholder [## 具体的な意図を追記 ##]\n
        - "MissingContext": placeholder [## 背景情報を追記 ##]\n
        - "UnclearActorObject": placeholder [## 主語・目的語を明確化 ##]\n
        - "UnexplainedTerminology": placeholder [## 専門用語の説明 ##]\n
      action:
        - "MissingDeadlinePriority": placeholder [## 期限を追記 ##]\n
        - "UnclearNextStep": 次の行動を明示（例：「次に〜をお願いします」）
        - "UnansweredQuestion": placeholder [## 質問への回答 ##]\n
        - "ResourceAuthorityUnclear": placeholder [## 必要なリソースや権限 ##]\n
